...
describe('API permissions', function(){
    ...
    describe('GET/assigned/:id', function(){
        ...
        xit('responds to an invalid id with 400', function() {
        });

        xit('responds with 403 when user is not authenticated', function() {
        });

        xit('responds to missing read permissions to the API with 403', function() {
        });

        xit('responds with 403 when the existing user group with the given id does not belong to the same client as the logged in user', function() {
        });

        xit('responds with 403 when the id is valid but does not exist', function() {
        });
        ...
        xit('returns an empty list when no permission was assigned to the user group', function() {
        });

        xit('returns a list of all assigned permissions for the user group when the modules of the permissions are available to the portal and to the client of the user group', function() {
        });
    ...
    describe('GET/available/:id', function(){
        ...
        xit('returns an empty list when all available permissions are already assigned to the user group', function() {
        });

        xit('returns a list of all permissions which were not assigned to the user group and which are available to the portal and to the client of the user group', function() {
        });

        xit('returns a list of all permissions which were not assigned to the user group and which are available to the portal and to the client of the user group and additionally with the selected permission key', function() {
        });
    ...
    describe('POST/', function(){
        ...
        it('responds with correct permission data with inserted new permission containing an _id field', function(done) {
        ...
        it('responds with arbitrary clientId with inserted new permission containing clientId field corresponding to clientId of logged-in user ', function(done) {
            db.get('users').findOne({name: '0_0_0'}).then(function(userFromDatabase){ //request user different than logged-in user to get clientId
        ...
        xit('responds with 409 when there is already a permission for the given permission key and user group', function() {
            // Create a permission to the activities for an user group (read)
            // Try to create another permission also for activities for the same user group (write), this must not be possible
        });
