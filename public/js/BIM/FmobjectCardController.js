app.controller('BIMFmobjectCardController', function($scope, $rootScope, $http, $mdDialog, $element, $mdToast, $mdPanel, $translate, $mdDialog, utils) {
    ...
    $scope.createFmObject = function() {
        ...
        var fmObjectToSend =  { 
            label: $scope.fmObject.label,
            previewimagedocumentname: $scope.fmObject.previewImageId
        };
        $http.post('/api/dynamic/' + $scope.fmObject.datatypename, fmObjectToSend).then(function(response) {
    ...
    $scope.load = function() {
        ...
        $http.get("/api/datatypes?forlist=fmobjects").then(function(datatyperesponse) {
            $scope.datatypes = datatyperesponse.data;
            if ($scope.params.name) {
                ...
                if ($scope.params.parentname && $scope.params.parentdatatypename) {
                    $scope.fmObject.parentname = $scope.params.parentname;
                    $scope.fmObject.parentdatatypename = $scope.params.parentdatatypename;
                }
                $rootScope.isLoading = false;
            }
