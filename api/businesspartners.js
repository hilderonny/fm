...
router.get('/forIds', auth(false, false, co.modules.businesspartner), (req, res) => {
    ...
    auth.canAccess(req.user._id, co.permissions.CRM_BUSINESSPARTNERS, 'r', co.modules.businesspartners, req.db).then(function(accessAllowed) {
        ...
        req.db.get(co.collections.businesspartners).find({
            _id: { $in: ids },
            clientId: clientId,
        }).then((partner) => {
            res.send(partner);
 ...
 */
router.get('/:id', auth(co.permissions.CRM_BUSINESSPARTNERS, 'r', co.modules.businesspartners), validateId,validateSameClientId(co.collections.businesspartners), (req, res) => {
    req.db.get(co.collections.businesspartners).findOne(req.params.id, req.query.fields).then((partner) => {
        res.send(partner);
    });
});
/**
...
router.post('/', auth(co.permissions.CRM_BUSINESSPARTNERS, 'w', co.modules.businesspartners),function(req, res){
    ...
    req.db.insert(co.collections.businesspartners, partner).then((insertedPartner) => {
 ...
 */
router.put('/:id' , auth(co.permissions.CRM_BUSINESSPARTNERS, 'w', co.modules.businesspartners), validateId, validateSameClientId(co.collections.businesspartners), function(req,res){
    var partner = req.body;
    if(!partner || Object.keys(partner).length < 1) {
        return res.sendStatus(400);
    }
    ...
    req.db.update(co.collections.businesspartners, req.params.id, { $set: partner }).then((updatedParnter) => {
